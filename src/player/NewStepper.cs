...
public partial class NewStepper : Node3D {
  [ExportCategory("Stepper")] [Export] private bool _enableStepper = true;
  ...
  [ExportSubgroup("Stepper projection")] [Export(PropertyHint.Range, "0.05f, 2f")]
  private float _projectionMaxDistance;

  [Export(PropertyHint.Range, "0.1f, 3f")]
  private float _stepHeight;

  private float _stepRayDistance;

  [Export(PropertyHint.Range, "0.05f, 0.2f")]
  private float _maxElevation;

  [ExportSubgroup("Projection Smoother")] [Export]
  private float _smoothingSpeed = 5.0f;

  [ExportSubgroup("Gizmos")] [Export] private MeshInstance3D _pointGizmo;
