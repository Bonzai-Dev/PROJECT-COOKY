...
public partial class PlayerMovement : CharacterBody3D {
  ...
  [ExportCategory("Movement")] [Export] public float walkingSpeed { private set; get; } = 5.0f;
  ...
  [Export]
  public float lerpSpeed { private set; get; } = 10.0f; // Gradually changes a value. (Adding smoothing to values)

  [Export] private float _airLerpSpeed = 3.0f;

  [ExportCategory("Jumping")] [Export] private float _jumpVelocity = 4.5f;
  ...
  [ExportSubgroup("Z Tilt")] [Export] private float _zRotationLerp = 7f;
  ...
  [ExportSubgroup("Crouching")] [Export] private CollisionShape3D _crouchingCollider;
  [Export] public RayCast3D ceilingRay { private set; get; }
  [Export] public float crouchingSpeed { private set; get; } = 3.0f;

  [Export(PropertyHint.Range, "0.25f, 0.75f")]
  private float _crouchingDepth = 0.5f;

  private float _initialDepth;


  [ExportSubgroup("Sliding")] [Export] public float slideTimerMax { private set; get; } = 1.0f;
  ...
  [ExportSubgroup("Vaulting")] [Export] private RayCast3D _vaultRay;
  ...
  [Export]
  public float wallRunTime { private set; get; } = 3f;
  ...
  [Export]
  public float verticalRunHeight { private set; get; } = 4f; // in metres

  [ExportSubgroup("Head Bobbing")] [Export]
  private float _headBobSprintSpeed = 22.0f;
  ...
  [ExportSubgroup("Sensitivity")] [Export(PropertyHint.Range, "0, 1,")]
  public float mouseSensitivityX = 0.4f;

  [Export(PropertyHint.Range, "0, 1,")] public float mouseSensitivityY = 0.4f;


  [ExportSubgroup("Free Looking")] [Export]
  private float _freeLookTilt = 0.3f;

  [ExportSubgroup("Field of View")] [Export]
  ...
  [ExportSubgroup("Interface")] [Export] private Label _speedLabel;
  ...
  public void RotatePlayerByConstraintUp(float mouseX, float mouseY, float leftDeg, float rightDeg, float upDeg,
    float downDeg) {
    float rotationY = Mathf.DegToRad(-mouseX * mouseSensitivityX);
    rotationY = Mathf.Clamp(rotationY, Mathf.DegToRad(leftDeg), Mathf.DegToRad(rightDeg));
    ...
    if (_animator != null) {
      _animator.Set("parameters/moveState/conditions/idle", FSM.CurrentState is PlayerIdle
                                                            || (inputDirection.Length() <= 0.1f
                                                                && (FSM.CurrentState is not PlayerWallrun ||
                                                                    FSM.CurrentState is not PlayerVerticalWallrun)));

      _animator.Set("parameters/moveState/conditions/moving", IsOnFloor() && (FSM.CurrentState is PlayerWalk
                                                                              || FSM.CurrentState is PlayerSprint ||
      ...
      if (_animationLabel != null) {
        AnimationNodeStateMachinePlayback node =
          (AnimationNodeStateMachinePlayback)_animator.Get("parameters/moveState/playback");
  ...
  private void HandleJump(float jumpSpeed) {
    ...
    if (Input.IsActionJustPressed("movement_jump") && ((IsOnFloor() && !ceilingRay.IsColliding()) || airTime < _coyoteTime)
    ...
    else if (Input.IsActionJustPressed("movement_jump") && _jumpsDone > 0 && _jumpsDone < _jumps) {
      // Jump
      Jump(jumpSpeed);
    }
  }
  ...
  public bool CheckWall(out KinematicCollision3D collision, out String direction) {
    ...
    foreach (KinematicCollision3D c in collisions) {
      ...
      float dotCollision =
        Mathf.Abs(collisionNormal.Dot(Vector3.Up)); // Get the dot product to see if the wall is side ways
  ...
  public override void _PhysicsProcess(double delta) {
    ...
    if ((IsOnFloor() && FSM.CurrentState is not PlayerSlide && inputDir != Vector2.Zero)) {
      ...
      eyes.Y = Mathf.Lerp(eyes.Y, _headBobVector.Y * (_headBobCurrentIntensity / 2.0f),
        1.0f - Mathf.Pow(0.5f, (float)delta * lerpSpeed));
      eyes.X = Mathf.Lerp(eyes.X, _headBobVector.X * _headBobCurrentIntensity,
        1.0f - Mathf.Pow(0.5f, (float)delta * lerpSpeed));
